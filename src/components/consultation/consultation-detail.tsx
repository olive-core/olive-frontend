  ...
  return (
    <div className="mx-auto mb-4 w-full max-w-xl px-4">
      <div className="flex items-center justify-between gap-1 rounded-xl border border-slate-200 bg-white p-1 shadow-sm">
        <Button variant="ghost" size="sm" aria-label="Previous consultation" disabled={!previousPrescriptionId} onClick={() => previousPrescriptionId && openConsultation(previousPrescriptionId)}>
        ...
        <Button variant="ghost" size="sm" aria-label="Next consultation" disabled={!nextPrescriptionId} onClick={() => nextPrescriptionId && openConsultation(nextPrescriptionId)}>
...
function BackButton({ caseId }: { caseId?: string | null }) {
  const navigate = useNavigate()
  return (
    <Button
      variant="ghost"
      size="sm"
      onClick={() => caseId ? navigate({ to: '/doctor/cases/$caseId', params: { caseId } }) : navigate({ to: '/doctor/consultations' })}
      className="text-slate-500 hover:text-slate-800 -ml-2"
    >
      <ArrowLeftIcon className="size-4 mr-1" />
      {caseId ? 'Back to case' : 'Consultations'}
    </Button>
  )
}
...
function DetailToolbar({
  ...
  return (
    <div className="container mx-auto mt-4 mb-6 flex flex-wrap items-center justify-between gap-2 px-4 print:hidden">
      {!embedded && <BackButton caseId={caseId} />}
      {!embedded && caseCode && <CaseCodeButton code={caseCode} />}
...
export default function ConsultationDetailContent({ prescriptionId, initialDocument, embedded = false }: { prescriptionId: string; initialDocument?: 'notes'; embedded?: boolean }) {
  ...
  const [activeDocument, setActiveDocument] = useState<PrescriptionDocument>(
    initialDocument === 'notes' ? 'notes' : 'prescription'
  )
  ...
  // Case participants can read the series; only the consultation author can edit notes.
    ...
    <>
      ...
      <div className="print:hidden">
        <DetailToolbar
          ...
          access={embedded ? undefined : accessBadge}
          embedded={embedded}
          caseId={consultation.case_root_session_id}
          caseCode={consultation.case_code}
          onShare={!embedded && canShare ? () => setShareDialogOpen(true) : undefined}
          followUpAction={!embedded && canFollowUp && consultation.session_id && !consultation.has_follow_up ? (
            <Button
